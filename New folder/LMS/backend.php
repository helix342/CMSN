...
function getcourses($semester)
{
    ...
    if ($department && $semester) {
        $query = "SELECT course_id, course_name, description , academic_year ,syllabus FROM course WHERE department = ? AND semester = ? AND status = 'approved'";
        ...
        echo json_encode(['error' =>   'Invalid department or year']);
    }
}
    ...
    switch ($action) {
        ...
        case 'getcourse':
            if (isset($_GET['semester'])) {
                ...
                $response = getcourses($semester);
            } else {
                $response = json_encode(['error' => 'Semester is missing.']);
            }
            break;
        case 'getunits':
            if (isset($_POST['course_id'])) {
...
// // Add a direct file serving route (to maintain compatibility with existing code)
// if (isset($_GET['file'])) {
//     serveFile($_GET['file']);
// }